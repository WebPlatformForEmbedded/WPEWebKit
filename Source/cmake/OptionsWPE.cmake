...
WEBKIT_OPTION_DEFINE(USE_SYSPROF_CAPTURE "Whether to use libsysprof-capture for tracing." PUBLIC OFF)
...
if (WPE_API_VERSION VERSION_EQUAL "1.1")
    CALCULATE_LIBRARY_VERSIONS_FROM_LIBTOOL_TRIPLE(WEBKIT 8 11 8)
else ()
    CALCULATE_LIBRARY_VERSIONS_FROM_LIBTOOL_TRIPLE(WEBKIT 5 11 4)
endif ()
...
option(USE_MANETTE_GAMEPAD_PROVIDER "Use ManetteGamepadProvider (direct libmanette) instead of libwpe gamepad" OFF)

if (ENABLE_GAMEPAD)
    if (USE_MANETTE_GAMEPAD_PROVIDER)
        find_package(Manette)
        if (NOT Manette_FOUND)
            message(FATAL_ERROR "libmanette is required for USE_MANETTE_GAMEPAD_PROVIDER")
        endif ()
...
if (ENABLE_SPEECH_SYNTHESIS)
    if (USE_FLITE)
        if (USE_TTS_CLIENT)
            message(FATAL_ERROR "USE_FLITE and USE_TTS_CLIENT cannot be enabled at the same time")
        endif ()
        find_package(Flite 2.2)
        if (NOT Flite_FOUND)
            message(FATAL_ERROR "Flite is needed for ENABLE_SPEECH_SYNTHESIS")
        endif ()
    elseif (NOT USE_TTS_CLIENT)
        message(FATAL_ERROR "Either USE_FLITE or USE_TTS_CLIENT must be enabled for ENABLE_SPEECH_SYNTHESIS")
    endif()
endif ()
