...
#include "config.h"
#include "MediaPlayerPrivateGStreamerPlaybin3.h"
...
#include "AudioTrackPrivateGStreamerPlaybin3.h"
#include "InbandMetadataTextTrackPrivateGStreamer.h"
#include "InbandTextTrackPrivateGStreamer.h"
#include "TextCombinerGStreamer.h"
#include "TextSinkGStreamer.h"
#include "VideoTrackPrivateGStreamerPlaybin3.h"
#endif
...
static void busMessageCallback(GstBus*, GstMessage* message, MediaPlayerPrivateGStreamerPlaybin3* player)
{
    player->handleMessage(message);
}

void MediaPlayerPrivateGStreamerPlaybin3::setAudioStreamPropertiesCallback(MediaPlayerPrivateGStreamerPlaybin3* player, GObject* object)
{
    player->setAudioStreamProperties(object);
}

void MediaPlayerPrivateGStreamerPlaybin3::setAudioStreamProperties(GObject* object)
{
...
void MediaPlayerPrivateGStreamerPlaybin3::registerMediaEngine(MediaEngineRegistrar registrar)
{
    if (isAvailable()) {
        GST_INFO("Allow using MediaPlayerPrivateGStreamerPlaybin3");
        registrar([](MediaPlayer* player) { return std::make_unique<MediaPlayerPrivateGStreamerPlaybin3>(player); },
            getSupportedTypes, supportsType, nullptr, nullptr, nullptr, supportsKeySystem);
    }
}

bool MediaPlayerPrivateGStreamerPlaybin3::isAvailable()
{
    if (!MediaPlayerPrivateGStreamerPlaybin3Base::initializeGStreamerAndRegisterWebKitElements())
        return false;

    GRefPtr<GstElementFactory> factory = adoptGRef(gst_element_factory_find("playbin3"));
    return factory;
}

MediaPlayerPrivateGStreamerPlaybin3::MediaPlayerPrivateGStreamerPlaybin3(MediaPlayer* player)
    : MediaPlayerPrivateGStreamerPlaybin3Base(player)
    , m_buffering(false)
    ...
    , m_fillTimer(*this, &MediaPlayerPrivateGStreamerPlaybin3::fillTimerFired)
    ...
    , m_hasVideo(false)
    , m_hasAudio(false)
    , m_readyTimerHandler(RunLoop::main(), this, &MediaPlayerPrivateGStreamerPlaybin3::readyTimerFired)
    , m_totalBytes(0)
...
MediaPlayerPrivateGStreamerPlaybin3::~MediaPlayerPrivateGStreamerPlaybin3()
{
...
void MediaPlayerPrivateGStreamerPlaybin3::setPlaybinURL(const URL& url)
{
...
void MediaPlayerPrivateGStreamerPlaybin3::load(const String& urlString)
{
    loadFull(urlString, nullptr, String());
}

void MediaPlayerPrivateGStreamerPlaybin3::loadFull(const String& urlString, const gchar* playbinName,
...
#if ENABLE(MEDIA_SOURCE)
void MediaPlayerPrivateGStreamerPlaybin3::load(const String&, MediaSourcePrivateClient*)
{
    // Does not support anything bu MediaStream.
...
void MediaPlayerPrivateGStreamerPlaybin3::load(MediaStreamPrivate& stream)
{
...
void MediaPlayerPrivateGStreamerPlaybin3::commitLoad()
{
...
MediaTime MediaPlayerPrivateGStreamerPlaybin3::playbackPosition() const
{
...
void MediaPlayerPrivateGStreamerPlaybin3::readyTimerFired()
{
    GST_DEBUG("In READY for too long. Releasing pipeline resources.");
    changePipelineState(GST_STATE_NULL);
}

bool MediaPlayerPrivateGStreamerPlaybin3::changePipelineState(GstState newState)
{
    ASSERT(m_pipeline);
...
void MediaPlayerPrivateGStreamerPlaybin3::prepareToPlay()
{
...
void MediaPlayerPrivateGStreamerPlaybin3::play()
{
    if (!m_playbackRate) {
...
void MediaPlayerPrivateGStreamerPlaybin3::pause()
{
    m_playbackRatePause = false;
...
MediaTime MediaPlayerPrivateGStreamerPlaybin3::durationMediaTime() const
{
...
MediaTime MediaPlayerPrivateGStreamerPlaybin3::currentMediaTime() const
{
    if (!m_pipeline || m_errorOccured)
        return MediaTime::invalidTime();
...
void MediaPlayerPrivateGStreamerPlaybin3::seek(const MediaTime& mediaTime)
{
    if (!m_pipeline)
        return;
...
bool MediaPlayerPrivateGStreamerPlaybin3::doSeek(const MediaTime& position, float rate, GstSeekFlags seekType)
{
...
void MediaPlayerPrivateGStreamerPlaybin3::updatePlaybackRate()
{
    if (!m_changingRate)
        return;
...
bool MediaPlayerPrivateGStreamerPlaybin3::paused() const
{
    if (m_isEndReached) {
...
bool MediaPlayerPrivateGStreamerPlaybin3::seeking() const
{
    return m_seeking;
}
...
void MediaPlayerPrivateGStreamerPlaybin3::clearTracks()
{
#if ENABLE(VIDEO_TRACK)
...
#define CREATE_TRACK(type, Type) \
    m_has##Type = true; \
    if (!useMediaSource) {\
        RefPtr<Type##TrackPrivateGStreamerPlaybin3> track = Type##TrackPrivateGStreamerPlaybin3::create(createWeakPtr(), i, stream); \
...
FloatSize MediaPlayerPrivateGStreamerPlaybin3::naturalSize() const
{
#if ENABLE(MEDIA_STREAM)
    if (!m_isLegacyPlaybin && !m_currentVideoStreamId.isEmpty()) {
        RefPtr<VideoTrackPrivateGStreamerPlaybin3> videoTrack = m_videoTracks.get(m_currentVideoStreamId);
    ...
    return MediaPlayerPrivateGStreamerPlaybin3Base::naturalSize();
}
#else
#define CREATE_TRACK(type, _id, tracks, method, stream) m_has##Type## = true;
#endif // ENABLE(VIDEO_TRACK)

void MediaPlayerPrivateGStreamerPlaybin3::updateTracks()
{
...
#if 0 // FIXME(backport webrtc): We do not handle text tracks yet.
...
void MediaPlayerPrivateGStreamerPlaybin3::enableTrack(TrackPrivateBaseGStreamer::TrackType trackType, unsigned index)
{
...
void MediaPlayerPrivateGStreamerPlaybin3::videoChangedCallback(MediaPlayerPrivateGStreamerPlaybin3* player)
{
...
void MediaPlayerPrivateGStreamerPlaybin3::notifyPlayerOfVideo()
{
    ...
    for (gint i = 0; i < numTracks; ++i) {
        ...
        if (i < static_cast<gint>(m_videoTracks.size())) {
            RefPtr<VideoTrackPrivateGStreamerPlaybin3> existingTrack = m_videoTracks.get(streamId);
            if (existingTrack) {
        ...
        RefPtr<VideoTrackPrivateGStreamerPlaybin3> track = VideoTrackPrivateGStreamerPlaybin3::create(createWeakPtr(), i, pad);
...
void MediaPlayerPrivateGStreamerPlaybin3::videoSinkCapsChangedCallback(MediaPlayerPrivateGStreamerPlaybin3* player)
{
...
void MediaPlayerPrivateGStreamerPlaybin3::notifyPlayerOfVideoCaps()
{
    m_videoSize = IntSize();
    m_player->client().mediaPlayerEngineUpdated(m_player);
}

void MediaPlayerPrivateGStreamerPlaybin3::audioChangedCallback(MediaPlayerPrivateGStreamerPlaybin3* player)
{
...
void MediaPlayerPrivateGStreamerPlaybin3::notifyPlayerOfAudio()
{
    if (UNLIKELY(!m_pipeline || !m_source))
        return;
    ...
    for (gint i = 0; i < numTracks; ++i) {
        ...
        if (i < static_cast<gint>(m_audioTracks.size())) {
            RefPtr<AudioTrackPrivateGStreamerPlaybin3> existingTrack = m_audioTracks.get(streamId);
            if (existingTrack) {
                existingTrack->setIndex(i);
                if (existingTrack->pad() == pad)
                    continue;
        ...
        RefPtr<AudioTrackPrivateGStreamerPlaybin3> track = AudioTrackPrivateGStreamerPlaybin3::create(createWeakPtr(), i, pad);
        ASSERT(streamId == track->id());
...
#if ENABLE(VIDEO_TRACK)
void MediaPlayerPrivateGStreamerPlaybin3::textChangedCallback(MediaPlayerPrivateGStreamerPlaybin3* player)
{
...
void MediaPlayerPrivateGStreamerPlaybin3::notifyPlayerOfText()
{
    if (UNLIKELY(!m_pipeline || !m_source))
        return;
...
GstFlowReturn MediaPlayerPrivateGStreamerPlaybin3::newTextSampleCallback(MediaPlayerPrivateGStreamerPlaybin3* player)
{
    player->newTextSample();
    return GST_FLOW_OK;
}

void MediaPlayerPrivateGStreamerPlaybin3::newTextSample()
{
    if (!m_textAppSink)
        return;
...
void MediaPlayerPrivateGStreamerPlaybin3::setRate(float rate)
{
...
double MediaPlayerPrivateGStreamerPlaybin3::rate() const
{
    return m_playbackRate;
}

void MediaPlayerPrivateGStreamerPlaybin3::setPreservesPitch(bool preservesPitch)
{
    m_preservesPitch = preservesPitch;
}

std::unique_ptr<PlatformTimeRanges> MediaPlayerPrivateGStreamerPlaybin3::buffered() const
{
...
void MediaPlayerPrivateGStreamerPlaybin3::handleMessage(GstMessage* message)
{
    ...
    case GST_MESSAGE_ELEMENT:
    #if 0 // FIXME(webrtc backport) - we do not care about missing plugins here.
...
#endif
#if 0 // FIXME(Backport werbtc) ENABLE(ENCRYPTED_MEDIA)
        if (gst_structure_has_name(structure, "drm-key-needed")) {
            ...
            MediaPlayerPrivateGStreamerPlaybin3Base::dispatchCDMInstance();
        }
#endif
...
void MediaPlayerPrivateGStreamerPlaybin3::processBufferingStats(GstMessage* message)
{
    if (isLiveStream())
        return;
...
#if ENABLE(VIDEO_TRACK) && USE(GSTREAMER_MPEGTS)
void MediaPlayerPrivateGStreamerPlaybin3::processMpegTsSection(GstMpegtsSection* section)
{
    ASSERT(section);
...
#if ENABLE(VIDEO_TRACK)
void MediaPlayerPrivateGStreamerPlaybin3::processTableOfContents(GstMessage* message)
{
    if (m_chaptersTrack)
...
void MediaPlayerPrivateGStreamerPlaybin3::processTableOfContentsEntry(GstTocEntry* entry)
{
    ASSERT(entry);
...
void MediaPlayerPrivateGStreamerPlaybin3::purgeInvalidAudioTracks(Vector<String> validTrackIds)
{
...
void MediaPlayerPrivateGStreamerPlaybin3::purgeInvalidVideoTracks(Vector<String> validTrackIds)
{
...
void MediaPlayerPrivateGStreamerPlaybin3::purgeInvalidTextTracks(Vector<String> validTrackIds)
{
...
void MediaPlayerPrivateGStreamerPlaybin3::fillTimerFired()
{
    GstQuery* query = gst_query_new_buffering(GST_FORMAT_PERCENT);
...
MediaTime MediaPlayerPrivateGStreamerPlaybin3::maxMediaTimeSeekable() const
{
    if (m_errorOccured)
...
MediaTime MediaPlayerPrivateGStreamerPlaybin3::maxTimeLoaded() const
{
    if (m_errorOccured)
        return MediaTime::zeroTime();
...
bool MediaPlayerPrivateGStreamerPlaybin3::didLoadingProgress() const
{
...
unsigned long long MediaPlayerPrivateGStreamerPlaybin3::totalBytes() const
{
    if (m_errorOccured)
        return 0;
...
void MediaPlayerPrivateGStreamerPlaybin3::sourceSetupCallback(MediaPlayerPrivateGStreamerPlaybin3* player, GstElement* sourceElement)
{
    player->sourceSetup(sourceElement);
}

void MediaPlayerPrivateGStreamerPlaybin3::uriDecodeBinElementAddedCallback(GstBin* bin, GstElement* element, MediaPlayerPrivateGStreamerPlaybin3* player)
{
...
void MediaPlayerPrivateGStreamerPlaybin3::downloadBufferFileCreatedCallback(MediaPlayerPrivateGStreamerPlaybin3* player)
{
...
void MediaPlayerPrivateGStreamerPlaybin3::purgeOldDownloadFiles(const char* downloadFileTemplate)
{
...
void MediaPlayerPrivateGStreamerPlaybin3::sourceSetup(GstElement* sourceElement)
{
...
bool MediaPlayerPrivateGStreamerPlaybin3::hasSingleSecurityOrigin() const
{
    if (!m_source)
        return false;
...
void MediaPlayerPrivateGStreamerPlaybin3::cancelLoad()
{
...
void MediaPlayerPrivateGStreamerPlaybin3::asyncStateChangeDone()
{
    if (!m_pipeline || m_errorOccured)
        return;
...
void MediaPlayerPrivateGStreamerPlaybin3::updateStates()
{
    if (!m_pipeline)
        return;
...
bool MediaPlayerPrivateGStreamerPlaybin3::handleSyncMessage(GstMessage* message)
{
#if GST_CHECK_VERSION(1, 10, 0)
    ...
    return MediaPlayerPrivateGStreamerPlaybin3Base::handleSyncMessage(message);
}

void MediaPlayerPrivateGStreamerPlaybin3::mediaLocationChanged(GstMessage* message)
{
    if (m_mediaLocations)
        gst_structure_free(m_mediaLocations);
...
bool MediaPlayerPrivateGStreamerPlaybin3::loadNextLocation()
{
    if (!m_mediaLocations)
        return false;
...
void MediaPlayerPrivateGStreamerPlaybin3::loadStateChanged()
{
    updateStates();
}

void MediaPlayerPrivateGStreamerPlaybin3::timeChanged()
{
    updateStates();
    m_player->timeChanged();
}

void MediaPlayerPrivateGStreamerPlaybin3::didEnd()
{
...
void MediaPlayerPrivateGStreamerPlaybin3::durationChanged()
{
...
void MediaPlayerPrivateGStreamerPlaybin3::loadingFailed(MediaPlayer::NetworkState error)
{
...
static HashSet<String, ASCIICaseInsensitiveHash>& mimeTypeSet()
{
    ...
    {
        MediaPlayerPrivateGStreamerPlaybin3Base::initializeGStreamerAndRegisterWebKitElements();
...
void MediaPlayerPrivateGStreamerPlaybin3::getSupportedTypes(HashSet<String, ASCIICaseInsensitiveHash>& types)
{
    types = mimeTypeSet();
}

MediaPlayer::SupportsType MediaPlayerPrivateGStreamerPlaybin3::supportsType(const MediaEngineSupportParameters& parameters)
{
    MediaPlayer::SupportsType result = MediaPlayer::IsNotSupported;
#if ENABLE(MEDIA_SOURCE)
    // MediaPlayerPrivateGStreamerPlaybin3MSE is in charge of mediasource playback, not us.
...
void MediaPlayerPrivateGStreamerPlaybin3::setDownloadBuffering()
{
    if (!m_pipeline)
        return;
...
void MediaPlayerPrivateGStreamerPlaybin3::setPreload(MediaPlayer::Preload preload)
{
...
GstElement* MediaPlayerPrivateGStreamerPlaybin3::createAudioSink()
{
...
GstElement* MediaPlayerPrivateGStreamerPlaybin3::audioSink() const
{
    GstElement* sink;
...
#if ENABLE(WEB_AUDIO)
void MediaPlayerPrivateGStreamerPlaybin3::ensureAudioSourceProvider()
{
    if (!m_audioSourceProvider)
        m_audioSourceProvider = std::make_unique<AudioSourceProviderGStreamer>();
}

AudioSourceProvider* MediaPlayerPrivateGStreamerPlaybin3::audioSourceProvider()
{
    ensureAudioSourceProvider();
...
void MediaPlayerPrivateGStreamerPlaybin3::createGSTPlayBin(const gchar* playbinName, const String& pipelineName)
{
    if (m_pipeline) {
        if (!playbinName) {
...
void MediaPlayerPrivateGStreamerPlaybin3::simulateAudioInterruption()
{
...
bool MediaPlayerPrivateGStreamerPlaybin3::didPassCORSAccessCheck() const
{
    if (WEBKIT_IS_WEB_SRC(m_source.get()))
...
bool MediaPlayerPrivateGStreamerPlaybin3::canSaveMediaData() const
{
    if (isLiveStream())
        return false;
