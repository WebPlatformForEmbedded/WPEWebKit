...
#include "config.h"
#include "MediaPlayerPrivateGStreamerPlaybin3Base.h"
...
GST_DEBUG_CATEGORY(webkit_media_player_playbin3_debug);
#define GST_CAT_DEFAULT webkit_media_player_playbin3_debug
...
void registerWebKitGStreamerElements()
{
    ...
    GST_DEBUG_CATEGORY_INIT(webkit_media_player_playbin3_debug, "webkitmediaplayerplaybin3", 0, "WebKit media player playbin3");
}

bool MediaPlayerPrivateGStreamerPlaybin3Base::initializeGStreamerAndRegisterWebKitElements()
{
...
MediaPlayerPrivateGStreamerPlaybin3Base::MediaPlayerPrivateGStreamerPlaybin3Base(MediaPlayer* player)
    : m_notifier(MainThreadNotifier<MainThreadNotification>::create())
    , m_player(player)
    , m_fpsSink(nullptr)
    , m_readyState(MediaPlayer::HaveNothing)
    , m_networkState(MediaPlayer::Empty)
    , m_drawTimer(RunLoop::main(), this, &MediaPlayerPrivateGStreamerPlaybin3Base::repaint)
#if USE(TEXTURE_MAPPER_GL)
...
MediaPlayerPrivateGStreamerPlaybin3Base::~MediaPlayerPrivateGStreamerPlaybin3Base()
{
#if 0 // FIXME(Backport werbtc) ENABLE(ENCRYPTED_MEDIA)
...
void MediaPlayerPrivateGStreamerPlaybin3Base::setPipeline(GstElement* pipeline)
{
    m_pipeline = pipeline;

    GRefPtr<GstBus> bus = adoptGRef(gst_pipeline_get_bus(GST_PIPELINE(m_pipeline.get())));
    gst_bus_set_sync_handler(bus.get(), [](GstBus*, GstMessage* message, gpointer userData) {
        auto& player = *static_cast<MediaPlayerPrivateGStreamerPlaybin3Base*>(userData);
...
bool MediaPlayerPrivateGStreamerPlaybin3Base::handleSyncMessage(GstMessage* message)
{
...
#if USE(GSTREAMER_GL)
GstContext* MediaPlayerPrivateGStreamerPlaybin3Base::requestGLContext(const char* contextType)
{
...
bool MediaPlayerPrivateGStreamerPlaybin3Base::ensureGstGLContext()
{
    if (m_glContext)
        return true;
...
FloatSize MediaPlayerPrivateGStreamerPlaybin3Base::naturalSize() const
{
    if (!hasVideo())
        return FloatSize();
...
void MediaPlayerPrivateGStreamerPlaybin3Base::setVolume(float volume)
{
    if (!m_volumeElement)
        return;
...
float MediaPlayerPrivateGStreamerPlaybin3Base::volume() const
{
    if (!m_volumeElement)
        return 0;
...
void MediaPlayerPrivateGStreamerPlaybin3Base::notifyPlayerOfVolumeChange()
{
...
void MediaPlayerPrivateGStreamerPlaybin3Base::volumeChangedCallback(MediaPlayerPrivateGStreamerPlaybin3Base* player)
{
...
MediaPlayer::NetworkState MediaPlayerPrivateGStreamerPlaybin3Base::networkState() const
{
    return m_networkState;
}

MediaPlayer::ReadyState MediaPlayerPrivateGStreamerPlaybin3Base::readyState() const
{
    return m_readyState;
}

void MediaPlayerPrivateGStreamerPlaybin3Base::sizeChanged()
{
    notImplemented();
}

void MediaPlayerPrivateGStreamerPlaybin3Base::setMuted(bool mute)
{
    if (!m_volumeElement)
        return;
...
bool MediaPlayerPrivateGStreamerPlaybin3Base::muted() const
{
    if (!m_volumeElement)
        return false;
...
void MediaPlayerPrivateGStreamerPlaybin3Base::notifyPlayerOfMute()
{
    if (!m_player || !m_volumeElement)
        return;
...
void MediaPlayerPrivateGStreamerPlaybin3Base::muteChangedCallback(MediaPlayerPrivateGStreamerPlaybin3Base* player)
{
...
void MediaPlayerPrivateGStreamerPlaybin3Base::acceleratedRenderingStateChanged()
{
    m_renderingCanBeAccelerated = m_player && m_player->client().mediaPlayerAcceleratedCompositingEnabled();
}

#if USE(TEXTURE_MAPPER_GL)
RefPtr<TextureMapperPlatformLayerProxy> MediaPlayerPrivateGStreamerPlaybin3Base::proxy() const
{
    return m_platformLayerProxy.copyRef();
}

void MediaPlayerPrivateGStreamerPlaybin3Base::swapBuffersIfNeeded()
{
}

void MediaPlayerPrivateGStreamerPlaybin3Base::pushTextureToCompositor()
{
#if !USE(GSTREAMER_GL)
...
void MediaPlayerPrivateGStreamerPlaybin3Base::repaint()
{
    ASSERT(m_sample);
    ASSERT(isMainThread());
...
void MediaPlayerPrivateGStreamerPlaybin3Base::triggerRepaint(GstSample* sample)
{
    bool triggerResize;
    {
        auto sampleLocker = holdLock(m_sampleMutex);
    ...
    if (triggerResize) {
        GST_DEBUG_BIN_TO_DOT_FILE_WITH_TS(GST_BIN(m_pipeline.get()), GST_DEBUG_GRAPH_SHOW_ALL, "webkit-video.triggerResize");
...
void MediaPlayerPrivateGStreamerPlaybin3Base::repaintCallback(MediaPlayerPrivateGStreamerPlaybin3Base* player, GstSample* sample)
{
    player->triggerRepaint(sample);
}

void MediaPlayerPrivateGStreamerPlaybin3Base::cancelRepaint(bool destroying)
{
...
void MediaPlayerPrivateGStreamerPlaybin3Base::repaintCancelledCallback(MediaPlayerPrivateGStreamerPlaybin3Base* player)
{
    player->cancelRepaint();
}

#if USE(GSTREAMER_GL)
GstFlowReturn MediaPlayerPrivateGStreamerPlaybin3Base::newSampleCallback(GstElement* sink, MediaPlayerPrivateGStreamerPlaybin3Base* player)
{
...
GstFlowReturn MediaPlayerPrivateGStreamerPlaybin3Base::newPrerollCallback(GstElement* sink, MediaPlayerPrivateGStreamerPlaybin3Base* player)
{
...
void MediaPlayerPrivateGStreamerPlaybin3Base::flushCurrentBuffer()
{
...
void MediaPlayerPrivateGStreamerPlaybin3Base::setSize(const IntSize& size)
{
    m_size = size;
}

void MediaPlayerPrivateGStreamerPlaybin3Base::paint(GraphicsContext& context, const FloatRect& rect)
{
...
#if USE(GSTREAMER_GL)
bool MediaPlayerPrivateGStreamerPlaybin3Base::copyVideoTextureToPlatformTexture(GraphicsContext3D* context, Platform3DObject outputTexture, GC3Denum outputTarget, GC3Dint level, GC3Denum internalFormat, GC3Denum format, GC3Denum type, bool premultiplyAlpha, bool flipY)
{
...
NativeImagePtr MediaPlayerPrivateGStreamerPlaybin3Base::nativeImageForCurrentTime()
{
#if USE(CAIRO) && ENABLE(ACCELERATED_2D_CANVAS)
    if (m_usingFallbackVideoSink)
        return nullptr;
...
void MediaPlayerPrivateGStreamerPlaybin3Base::setVideoSourceOrientation(const ImageOrientation& orientation)
{
...
bool MediaPlayerPrivateGStreamerPlaybin3Base::supportsFullscreen() const
{
    return true;
}

MediaPlayer::MovieLoadType MediaPlayerPrivateGStreamerPlaybin3Base::movieLoadType() const
{
...
#if USE(GSTREAMER_GL)
GstElement* MediaPlayerPrivateGStreamerPlaybin3Base::createGLAppSink()
{
    ...
    g_object_set(appsink, "enable-last-sample", FALSE, "emit-signals", TRUE, "max-buffers", 1, "sync", FALSE, nullptr);
...
GstElement* MediaPlayerPrivateGStreamerPlaybin3Base::createVideoSinkGL()
{
    if (!webkitGstCheckVersion(1, 8, 0))
        return nullptr;
...
void MediaPlayerPrivateGStreamerPlaybin3Base::ensureGLVideoSinkContext()
{
...
GstElement* MediaPlayerPrivateGStreamerPlaybin3Base::createVideoSink()
{
...
void MediaPlayerPrivateGStreamerPlaybin3Base::setStreamVolumeElement(GstStreamVolume* volume)
{
...
unsigned MediaPlayerPrivateGStreamerPlaybin3Base::decodedFrameCount() const
{
...
unsigned MediaPlayerPrivateGStreamerPlaybin3Base::droppedFrameCount() const
{
...
unsigned MediaPlayerPrivateGStreamerPlaybin3Base::audioDecodedByteCount() const
{
...
unsigned MediaPlayerPrivateGStreamerPlaybin3Base::videoDecodedByteCount() const
{
    GstQuery* query = gst_query_new_position(GST_FORMAT_BYTES);
    gint64 position = 0;
...
#if 0 // FIXME(Backport werbtc) ENABLE(ENCRYPTED_MEDIA)
void MediaPlayerPrivateGStreamerPlaybin3Base::initializationDataEncountered(GstEvent* event)
{
...
void MediaPlayerPrivateGStreamerPlaybin3Base::cdmInstanceAttached(CDMInstance& instance)
{
...
void MediaPlayerPrivateGStreamerPlaybin3Base::cdmInstanceDetached(CDMInstance& instance)
{
#ifdef NDEBUG
...
void MediaPlayerPrivateGStreamerPlaybin3Base::attemptToDecryptWithInstance(CDMInstance& instance)
{
...
void MediaPlayerPrivateGStreamerPlaybin3Base::attemptToDecryptWithLocalInstance()
{
    // FIXME.
}

void MediaPlayerPrivateGStreamerPlaybin3Base::dispatchDecryptionKey(GstBuffer* buffer)
{
...
void MediaPlayerPrivateGStreamerPlaybin3Base::dispatchCDMInstance()
{
...
void MediaPlayerPrivateGStreamerPlaybin3Base::handleProtectionEvent(GstEvent* event)
{
...
bool MediaPlayerPrivateGStreamerPlaybin3Base::supportsKeySystem(const String& keySystem, const String& mimeType)
{
    bool result = false;
...
MediaPlayer::SupportsType MediaPlayerPrivateGStreamerPlaybin3Base::extendedSupportsType(const MediaEngineSupportParameters& parameters, MediaPlayer::SupportsType result)
{
