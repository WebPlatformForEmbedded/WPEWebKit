...
void GStreamerIncomingTrackProcessor::configure(GStreamerMediaEndpoint* endPoint, GRefPtr<GstPad>&& pad)
{
    m_endPoint = endPoint;
    m_pad = WTFMove(pad);
...
void GStreamerIncomingTrackProcessor::retrieveMediaStreamAndTrackIdFromSDP()
{
    ...
    g_object_get(m_endPoint->webrtcBin(), "remote-description", &description.outPtr(), nullptr);
...
{
    if (!m_endPoint || m_endPoint->isStopped())
        return;
    ...
    callOnMainThread([this] {
        if (!m_endPoint || m_endPoint->isStopped())
            return;
        m_endPoint->connectIncomingTrack(m_data);
    });
}
